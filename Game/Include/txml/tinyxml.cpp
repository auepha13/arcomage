...
TiXmlNode* TiXmlNode::LinkEndChild( TiXmlNode* node )
{
	...
	if (node->Type() == TiXmlNode::DOCUMENT) {
		delete node;
		if (GetDocument()) GetDocument()->
				SetError( TIXML_ERROR_DOCUMENT_TOP_ONLY, 0, 0,
...
TiXmlNode* TiXmlNode::ReplaceChild( TiXmlNode* replaceThis, const TiXmlNode&
	withThis )
{
...
const TiXmlNode* TiXmlNode::IterateChildren( const TiXmlNode*
	previous ) const
{
	if (!previous) {
		return FirstChild();
...
TiXmlDocument::TiXmlDocument( const TiXmlDocument&
	copy ) : TiXmlNode(TiXmlNode::DOCUMENT)
{
	copy.CopyTo( this );
}
